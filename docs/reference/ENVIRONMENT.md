...
| `PORTAL_DATABASE_URL`                  | _(default in example)_ | `src/lib/portalDb.ts`                                 | Customer Portal PostgreSQL connection string for subscription plan lookups.                                                      |
| `USER_RATE_LIMIT_ENABLED`              | `true`                 | `open-sse/services/userRateLimitManager.ts`           | Enable user subscription quota enforcement (toggles Pro/Pro Max limit checks).                                                   |
| `USER_RATE_LIMIT_FAIL_OPEN`            | `false`                | `open-sse/services/userRateLimitManager.ts`           | Fail-open behavior for subscription rate limiting when Redis/PostgreSQL is unavailable.                                          |
